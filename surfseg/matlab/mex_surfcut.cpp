...
{
	ensureOrError(8 <= nrhs && nrhs <= 10, "Must supply between 8 and 10 inputs");
	Volume<float> cost = getVolumeChecked<float>(prhs[0], "Cost volume");
	Volume<float> centers = getVolumeChecked<float>(prhs[1], "Centers");
	Volume<float> initRs = getVolumeChecked<float>(prhs[2], "initRs");
	...
	ensureOrError(mxGetN(prhs[1]) == 3 && isMatrix(prhs[1]), "Centers must be N x 3");

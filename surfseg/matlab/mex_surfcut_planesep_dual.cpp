...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
	...
	float smoothFactor = nrhs > 7 ? getCastScalarChecked<float>(prhs[8], "smoothFactor") : 0.0f;
	int smoothIter = nrhs > 8 ? getCastScalarChecked<int>(prhs[9], "smoothIter") : 0;
	...
	// Do the cut
	try {
		meshes = surfaceCutPlaneSepDualParallel(cost, std::move(meshes), numSamples, sampleStep, maxDiff, 
			costType, centerVec, connVec, 2);
	...
	for (int i = 0; i < nmesh; ++i) {
		...
		for (const auto& v : m.vertices) {
			vertData[i][v.self + 0 * numVerts] = m.vpos(v)[0];
			vertData[i][v.self + 1 * numVerts] = m.vpos(v)[1];
			vertData[i][v.self + 2 * numVerts] = m.vpos(v)[2];
		}
